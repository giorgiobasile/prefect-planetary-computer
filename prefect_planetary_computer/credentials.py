...
from prefect_planetary_computer.constants import CATALOG_URL
...
class PlanetaryComputerCredentials(Block):
    ...
    def get_gateway(self, **pc_gateway_kwargs: Dict) -> PlanetaryComputerGateway:
        """
        Provides a configured client for the PC Dask Gateway Server.

        Args:
            pc_gateway_kwargs: Keyword arguments to pass to the PC Gateway client.
        ...
        return PlanetaryComputerGateway(
            hub_api_token=self.hub_api_token.get_secret_value(),
            **pc_gateway_kwargs,
        )

    def new_cluster(
        self,
        **pc_gateway_cluster_kwargs: Dict,
    ) -> PlanetaryComputerCluster:
        """
        Instantiate a new cluster from the PC Dask Gateway Server.

        Args:
            pc_gateway_cluster_kwargs: keyword arguments to pass to the [`PlanetaryComputerCluster`](https://giorgiobasile.github.com/prefect-planetary-computer/gateway/#prefect_planetary_computer.gateway.PlanetaryComputerCluster) constructor.
        ...
        return PlanetaryComputerCluster(
            hub_api_token=self.hub_api_token.get_secret_value(),
            **pc_gateway_cluster_kwargs,
        )
